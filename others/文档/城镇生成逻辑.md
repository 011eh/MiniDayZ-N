...
> ⚠️ **关键：`roads_1/2` 只画「骨架」，不是最终连接。** 每行/列只连**前两个**地点，所以光看 grid 会有大量「孤立」地点。**真正的城镇↔道路连接发生在渲染阶段补臂**（见 §6 `generate_location`）：
> - **世界地图补臂**：渲染每个 block 时查 4 个正交邻格——**地点块**若邻格是道路(12/13/14)就朝它画一段路臂；**道路块**若邻格是地点/路(9/6/4/13/14/5/11/15)也朝它画臂。两边对接 → 紧邻道路的地点都会连上（验证自 `game_events.decoded.txt:2528-2552` 村庄块、`map_generation.decoded.txt:131-168` 道路块）。
> - **小地图支线**：`generate_minimap`（§7）给道路图标按邻格是否为地点/路**切换「带支线」的 AnimFrame**（基础帧 1 / 上支线 12 / 下支线 3…），所以小地图上能看到支路连到主干（`game_events.decoded.txt:12999+`）。
> - 推论：判断连通性**不能只数 `roads_1/2` 的骨架**，必须算上「地点-相邻道路」的补臂。
